...
// This is the prototype of the profit distributor token. This token is ERC20 compatible. 
// In terms of functionality, there are two main functions:
// distributeProfit: this function is called by the owner of the contract in a transaction with ether = (total tokens * profit per token).\
// 		     the parameters of this function is Wei per token (wpt).

// claimProfit: this function is called by token holders. The function transfers the holder's share of the profits to his/her address.

contract ProfitDistributorToken is SafeMath, Ownable, ERC20{
    ...
    event ProfitAvailable(uint256 profit);
    ...
    function moveToResiduals(address _addr) private {
        ...
        uint256 senderProfit = safeMul(wptUser , balances[_addr]);
        residuals[_addr] = safeAdd(residuals[_addr], senderProfit);
    ...
    function transfer(address _to, uint256 _value) 
	    ...
	    returns (bool success) {
	        ...
	        // move any profits for the sender and the receiver to their residuals
    ...
    function profitsOf(address _owner) 
        constant returns (uint256 profits) {
    ...
    function distributeProfit(uint256 wpt) public payable
        onlyOwner
        returns (bool success){
    ...
    function claimProfit() public
        ...
        hasTokens
        returns (bool success){
            uint256 profit = safeMul( safeSub(cumulative_wpt,cwpt_user[msg.sender]), balances[msg.sender]) + residuals[msg.sender];
